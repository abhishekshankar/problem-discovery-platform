"""SEC — press releases RSS (PRD §7.1; lightweight public signal)."""
...
class SECEdgarNewsCollector(Collector):
    ...
    def __init__(self, *, rss_url: str | None = None, max_items: int = 80) -> None:
        self.rss_url = rss_url or "https://www.sec.gov/news/pressreleases.rss"
    ...
    def fetch(self, since: datetime | None, run_id: str) -> Iterator[RawRecord]:
        r = requests.get(
            self.rss_url,
            timeout=60,
            headers={"User-Agent": "signal-problem-discovery/0.1 contact@localhost"},
        )
        r.raise_for_status()
